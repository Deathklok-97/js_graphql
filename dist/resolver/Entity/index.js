...
export default (function (entityName, data) {
  var entityFields = Object.keys(getFieldsFromEntities(data[entityName]));
  var manyToOneResolvers = entityFields.filter(isRelationshipField).reduce(function (resolvers, fieldName) {
    return Object.assign({}, resolvers, _defineProperty({}, getRelatedType(fieldName), function (entity) {
      return data[getRelatedKey(fieldName)].find(function (relatedRecord) {
        return relatedRecord.id == entity[fieldName];
      });
    }));
  }, {});
  var relatedField = getReverseRelatedField(entityName); // 'posts' => 'post_id'

  var hasReverseRelationship = function hasReverseRelationship(entityName) {
    return Object.keys(getFieldsFromEntities(data[entityName])).includes(relatedField);
  };
  ...
  var oneToManyResolvers = entities.filter(hasReverseRelationship).reduce(function (resolvers, entityName) {
    return Object.assign({}, resolvers, _defineProperty({}, getRelationshipFromKey(entityName), function (entity) {
